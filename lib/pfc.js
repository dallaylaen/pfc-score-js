...
const PFC = (function () {
    ...
    function Day (date) {
        ...
        this.addValue = function(raw) {
            if (!raw.time || !raw.name || raw.value === undefined)
                throw ("A daily value must have time, name, and value fields");

            if (!this.values[raw.name])
                this.values[raw.name] = [];
            this.values[raw.name].push( raw );
        };
        this.score = function() {
            let score = 0;
        ...
        this.load = function(raw) {
            ...
            for (let name in (raw.values || {})) {
                // TODO validate & deep copy
                this.values[name] = raw.values[name];
            }
            return this;
        };
    }
    ...
    function PFC () {
        ...
        this.addValue = function(raw) {
            // TODO value types
            const day = time2day(defaultDate(raw.time));
            this.getDay(day).addValue(raw);
